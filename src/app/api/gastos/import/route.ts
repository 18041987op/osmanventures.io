...
export async function POST(req: Request) {
  try {
    ...
    const b = (await req.json()) as { account_id?: string; file_hash?: string; filename?: string; rows?: Row[]; person?: string };
    ...
    const { data: rulesData } = await sb.from("category_rules").select("merchant,category").eq("owner", owner);
    ...
    const payload = await Promise.all(b.rows.map(async (r) => {
      ...
      return ({
        person: owner, account_id: b.account_id, type: txType, added_by: addedBy,
    ...
    await sb.from("uploads").insert({ owner, account_id: b.account_id, filename: b.filename || null, file_hash: b.file_hash || null, row_count: payload.length });
